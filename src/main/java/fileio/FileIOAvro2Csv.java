...
public class FileIOAvro2Csv {
    ...
    private static void runLocal() {
        ...
        csv2avro(s3o, //noOption,
                "C:\\Devx\\Tmp\\Resources\\persons.avro");
    }
    ...
    private static void csv2avro(PipelineOptions options, String pattern) {
        ...
        files.apply("", TextIO.readFiles())
                .apply("Read_Avro", AvroIO.readAllGenericRecords(Records.SCHEMA))
                .apply("filter_ignore_empty", ParDo.of(new DoFn<String, String>() {
                    @ProcessElement
                    public void onElement(@Element final GenericRecord element, final OutputReceiver<String> out) {
                        if (!element.isEmpty()) {
                            out.output(element);
                        }
                    }
