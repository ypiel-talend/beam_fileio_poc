...
public class WriteText {
    ...
    private static void displayLoadedModules() {
        List<Module> modules =
                ObjectMapper.findModules(ReflectHelpers.findClassLoader()); // To check if AWS module is loaded
    ...
    private static void runCountLocal() {
        ...
        runCount(s3o, //noOption,
                "C:\\Devx\\Tmp\\Resources\\persons.csv", "c:\\beamout\\writeText");
    }
    ...
    private static void runCountS3() {
        ...
        runCount(s3o, "s3://ypiel/beam/persons.csv", "s3://ypiel/beam/out/text/");
    }

    private static void runCount(PipelineOptions options, String pattern, String out) {
        ...
        p.apply("Configure", FileIO.match().filepattern(pattern))
                .apply("Select files", FileIO.readMatches())
                .apply("Read Files Content",
                        MapElements.into(TypeDescriptors.strings()).via((FileIO.ReadableFile f) -> {
                            try {
